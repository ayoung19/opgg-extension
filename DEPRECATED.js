...
if (window.location.pathname.indexOf("summoner/userName=") !== -1) {
    ...
    function start() {
        ...
        document.onclick = function(e) {
            ...
            if (mouseY > rect.top && mouseY < rect.bottom + 340 && mouseX > rect.left - 157 && mouseX < rect.right + 30) {} else {
        ...
        for (var p = 0; p < recentlyPlayedList.length; p++) {
            document.getElementById("recently-played-summoners").children[p].children[0].addEventListener("click", function() {
                document.getElementById("summonerNameInput").value = $(this).html();
                search();
                bankChange();
                document.getElementById("customContent").style.display = "none";
            })
        ...
        function bankChange() {
            ...
            if (document.getElementById("summonerNameInput").value != "") {
                ...
                for (var s = 1; s <= searchBank.length; s++) {
                    document.getElementById("full-list-summoners").children[s].children[0].addEventListener("click", function() {
                        document.getElementById("summonerNameInput").value = $(this).html();
                        search();
                        bankChange();
                        document.getElementById("customContent").style.display = "none";
                ...
                document.getElementById("full-list-summoners").children[0].children[0].addEventListener("click", function() {
                    for (var m = 0; m < numberOfGames; m++) {
                        document.getElementsByClassName("GameItemWrap")[m].setAttribute("style", "display:block");
                        document.getElementById("customContent").style.display = "none";
                        document.getElementById("summonerNameInput").value = "";
                        for (var i = 0; i < numberOfGames; i++) {
                            var teamCount = document.getElementsByClassName("FollowPlayers")[i].children.length;
                            for (var a = 0; a < teamCount; a++) {
                                var playerCount = document.getElementsByClassName("FollowPlayers")[i].children[a].children.length;
